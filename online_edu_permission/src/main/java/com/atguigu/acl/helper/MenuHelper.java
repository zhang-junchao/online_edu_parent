...
public class MenuHelper {
     ...
     * @return
     */
    public static List<JSONObject> bulid(List<TPermission> treeNodes) {
        List<JSONObject> meuns = new ArrayList<>();
        if(treeNodes.size() == 1) {
            TPermission topNode = treeNodes.get(0);
            //左侧一级菜单
            List<TPermission> oneMeunList = topNode.getChildren();
            for(TPermission one :oneMeunList) {
                ...
                List<TPermission> twoMeunList = one.getChildren();
                for(TPermission two :twoMeunList) {
                    ...
                    List<TPermission> threeMeunList = two.getChildren();
                    for(TPermission three :threeMeunList) {
